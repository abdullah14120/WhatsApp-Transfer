...
object NetworkTransferManager {
    ...
    private const val BUFFER_SIZE = 1048576 // 1MB Buffer لأقصى إنتاجية I/O

    suspend fun startReceiverServer(outputDir: File, onProgress: (String, Int) -> Unit) = withContext(Dispatchers.IO) {
    ...
    suspend fun sendFilesOverSocket(serverIp: String, sourceDir: File, onProgress: (Long, Long) -> Unit) = withContext(Dispatchers.IO) {
                    ...
                    RandomAccessFile(file, "r").use { raf ->
                        ...
                        BufferedOutputStream(socket.getOutputStream(), BUFFER_SIZE).use { bos ->
                            ...
                            while (true) {
                                ...
                                onProgress(sentBytesTotal, totalBytesAll)
                            }
                            bos.flush()
                        }
                    }
